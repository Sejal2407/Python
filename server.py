...
def receive_loop(listbox):
    while True:
        try:
            ...
            listbox.insert('end', "Client: " + decoded)

            # If client says bye, close gracefully
...
try:
    ...
    listbox.insert('end', f"--- Connected: {address} ---")
    ...
    messagebox.showerror("Connection Error", f"Could not start server:\n{e}")
    root.quit()
